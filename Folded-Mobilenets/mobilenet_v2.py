...
def mobilenet(input_tensor,
    ...
    Args:
        ...
        activation_fn: Activation function to use, defaults to tf.nn.relu6 if not
            specified.
        fold_indices: folded blocks indices
    ...
    for j in fold_indices:
        conv_defs['spec'][j+1].params['fold'] = True # +1 because first conv is not expanded_conv
...
mobilenet_v2_075_folded = wrapped_partial(mobilenet, depth_multiplier=0.75,
                                          fold_indices=[2,4],
                                          is_training=False,
                                          finegrain_classification_mode=True)
                                          # finegrain? TODO
mobilenet_v2_140_folded = wrapped_partial(mobilenet, depth_multiplier=1.4,
                                          fold_indices=[2,4],
                                          is_training=False)
mobilenet_v2_1_folded = wrapped_partial(mobilenet, depth_multiplier=1.,
                                        fold_indices=[2,4,8,9,10],
                                        is_training=False)
